// A4 invoice templates rendered as self-contained HTML strings for iframe printing.
// Each template returns a full <html> document sized to A4 (210mm × 297mm).
...
function money(n: number, d: InvoiceTemplateData) {
...
/* ---------------- CLASSIC ---------------- */
function classic(d: InvoiceTemplateData) {
  const css = `
    .head { display: flex; justify-content: space-between; align-items: flex-start; border-bottom: 3px solid #111827; padding-bottom: 10mm; }
    .co-name { font-size: 20px; font-weight: 700; letter-spacing: -0.01em; }
    .title { font-size: 32px; font-weight: 700; letter-spacing: 0.08em; color: #111827; text-transform: uppercase; }
    ...
    table.items thead th {
      background: #111827; color: #fff; text-align: left; font-weight: 600; font-size: 11px;
    ...
    .totals .grand { border-top: 2px solid #111827; margin-top: 4px; padding-top: 8px; font-size: 14px; font-weight: 700; }
    .footer { margin-top: auto; padding-top: 8mm; border-top: 1px solid #e5e7eb; font-size: 11px; color: #4b5563; }
    .status { display: inline-block; padding: 3px 10px; border-radius: 999px; font-size: 10px; text-transform: uppercase; letter-spacing: 0.06em; background:#fef3c7; color:#92400e; }
  `;
  const body = `
    <div class="page">
      <div class="head">
        ...
        <div style="text-align:right">
          ...
          <div class="meta"># ${esc(d.invoiceNo)}</div>
          <div class="meta">Issued ${esc(d.issueDate)}${d.dueDate ? ` &middot; Due ${esc(d.dueDate)}` : ""}</div>
      ...
      <div class="parties">
        ...
        <div class="box">
          ...
          <div class="small"><b>Invoice #</b> ${esc(d.invoiceNo)}</div>
          <div class="small"><b>Issue date</b> ${esc(d.issueDate)}</div>
          ${d.dueDate ? `<div class="small"><b>Due date</b> ${esc(d.dueDate)}</div>` : ""}
          <div class="small" style="margin-top:4px"><b>Amount due</b> ${esc(money(d.total - d.amountPaid, d))}</div>
        </div>
      </div>
      ...
      <div class="totals">
        ...
        <div class="row"><span class="muted">Paid</span><span>${esc(money(d.amountPaid, d))}</span></div>
        ${d.total - d.amountPaid > 0 ? `<div class="row" style="color:#b45309;font-weight:600"><span>Balance due</span><span>${esc(money(d.total - d.amountPaid, d))}</span></div>` : ""}
      </div>

      <div class="footer">
        ${d.notes ? `<div><b>Notes:</b> ${esc(d.notes)}</div>` : ""}
        ${d.footer ? `<div style="margin-top:6px">${esc(d.footer)}</div>` : `<div class="muted" style="margin-top:6px">Thank you for your business.</div>`}
      </div>
    </div>
  `;
...
/* ---------------- MODERN ---------------- */
function modern(d: InvoiceTemplateData) {
  const css = `
    .page { padding: 0 0 16mm 0; }
    .stripe { background: linear-gradient(135deg, #1e293b 0%, #0f172a 100%); color:#fff; padding: 14mm 14mm 10mm; }
    ...
    .meta { font-size: 11px; color:#cbd5e1; margin-top:4px; line-height:1.5; }
    ...
    table.items thead th {
      text-align: left; padding: 8px 6px; border-bottom: 2px solid #0f172a; font-size: 10px;
    ...
    .pay .label { color:#0f172a; }
    .totals { width: 70mm; font-size: 12px; }
    .totals .row { display:flex; justify-content:space-between; padding: 5px 0; }
    .totals .grand { background:#0f172a; color:#fff; padding: 10px 12px; border-radius: 4px; margin-top: 6px; font-weight:700; font-size: 14px; }
    .footer { margin-top: 10mm; padding: 6mm 14mm 0; border-top: 1px solid #e2e8f0; font-size: 11px; color:#475569; }
  `;
  const due = d.total - d.amountPaid;
  const body = `
    <div class="page">
      <div class="stripe">
        <div class="row">
      ...
      <div class="inner">
        <div class="parties">
          ...
          <div class="pcard">
            ...
            <div style="font-weight:700;font-size:18px">${esc(money(due, d))}</div>
            ${d.dueDate ? `<div class="small" style="color:#64748b">by ${esc(d.dueDate)}</div>` : ""}
          </div>
        </div>
        ...
        <div class="totals-wrap">
          <div class="pay">
            ${d.notes ? `<div class="label">Notes</div><div>${esc(d.notes)}</div>` : ""}
          </div>
          <div class="totals">
            <div class="row"><span class="muted">Subtotal</span><span>${esc(money(d.subtotal, d))}</span></div>
            ${d.discount > 0 ? `<div class="row"><span class="muted">Discount</span><span>-${esc(money(d.discount, d))}</span></div>` : ""}
            ...
            <div class="row" style="margin-top:6px"><span class="muted">Paid</span><span>${esc(money(d.amountPaid, d))}</span></div>
            ${due > 0 ? `<div class="row" style="color:#b45309;font-weight:600"><span>Due</span><span>${esc(money(due, d))}</span></div>` : ""}
          </div>
        </div>
      </div>

      <div class="footer">
        ${d.footer ? esc(d.footer) : "Thank you for your business. We appreciate your trust."}
      </div>
    </div>
  `;
  return wrap(css, body);
...
/* ---------------- MINIMAL ---------------- */
function minimal(d: InvoiceTemplateData) {
  const css = `
    ...
    .invoice-word { font-size: 11px; letter-spacing: 0.4em; text-transform: uppercase; color:#9ca3af; }
    .invoice-no { font-size: 22px; font-weight: 300; letter-spacing: -0.01em; margin-top: 2px; }
    ...
    <div class="page">
      <div class="top">
        ...
        <div style="text-align:right">
          ...
          <div class="invoice-no">${esc(d.invoiceNo)}</div>
        </div>
      </div>
      ...
      <div class="row3">
        ...
        <div>
          ...
          <div style="font-size:18px;font-weight:500">${esc(money(due, d))}</div>
        </div>
      </div>
          ...
          ${d.items.map(it => `
            <tr>
              <td>
                ...
                ${it.sku ? `<div style="color:#9ca3af;font-size:10px">${esc(it.sku)}</div>` : ""}
              </td>
              <td class="num">${esc(it.quantity)}</td>
              <td class="num">${esc(money(it.unit_price, d))}</td>
              <td class="num">${esc(money(it.total, d))}</td>
      ...
      <div class="totals">
        ...
        <div class="row grand"><span>Total</span><span>${esc(money(d.total, d))}</span></div>
        <div class="row" style="color:#6b7280;margin-top:4px"><span>Paid</span><span>${esc(money(d.amountPaid, d))}</span></div>
        ${due > 0 ? `<div class="row" style="color:#b45309"><span>Balance</span><span>${esc(money(due, d))}</span></div>` : ""}
      </div>

      ${d.notes ? `<div style="margin-top:10mm;color:#6b7280"><b style="color:#1f2937">Notes — </b>${esc(d.notes)}</div>` : ""}

      <div class="footer">
        ${d.footer ? esc(d.footer) : "Thank you."}
      </div>
    </div>
  `;
  return wrap(css, body);
...
export function renderInvoiceHTML(template: TemplateId, data: InvoiceTemplateData): string {
